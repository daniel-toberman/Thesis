# run_SRP_parametric.py - Version with SRP parameters as arguments
import argparse
import subprocess
import sys
...
def load_segment_multichannel(wav_ch1_path: Path, st: int, ed: int, example_idx=0, use_mic_id=[0, 1, 2, 3, 4, 5, 6, 7, 8], freq_min=300, freq_max=3000):
    """
...
def run_srp_return_details(fs: int, mic_signals: np.ndarray, use_mic_id: list,
                          srp_grid_cells: int, srp_mode: str, n_avg_samples: int,
                          n_dft_bins: int):
...
def plot_pairwise_cc_or_gcc(fs: int, X: np.ndarray, out: Path | None, use_gcc=True, n_dft_bins=1024):
    if plot_cc is None:
...
def process_row(row, idx, plots: bool, outdir: Path | None, save_cc: bool, save_gcc: bool, use_mic_id: list,
               srp_grid_cells: int, srp_mode: str, n_avg_samples: int, n_dft_bins: int,
    ...
    fs, X = load_segment_multichannel(wav_path, st, ed, example_idx=idx, use_mic_id=use_mic_id,
                                    freq_min=freq_min, freq_max=freq_max)  # Pass parameters explicitly
    if X.shape[0] != len(use_mic_id):
        return {"idx": idx, "filename": rel, "status": f"error: mic count mismatch ({X.shape[0]} vs {len(use_mic_id)})"}

    az, srp_obj, srp_map, grid = run_srp_return_details(fs, X, use_mic_id, srp_grid_cells,
    ...
    if plots:
        ...
        if save_gcc:
            plot_pairwise_cc_or_gcc(fs, X, outdir / f"gcc_pairwise_idx{idx}.png" if outdir else None, use_gcc=True, n_dft_bins=n_dft_bins)
        if save_cc:
            plot_pairwise_cc_or_gcc(fs, X, outdir / f"cc_pairwise_idx{idx}.png" if outdir else None, use_gcc=False, n_dft_bins=n_dft_bins)
...
def main():
    ...
    # SRP parameters (now as arguments instead of constants)
    p.add_argument("--srp_grid_cells", type=int, default=720,
                  help="Number of grid cells for SRP search (default: 720)")
    p.add_argument("--srp_mode", type=str, default="gcc_phat_freq",
                  choices=["gcc_phat_freq", "gcc_phat_time", "cross_correlation"],
                  help="SRP mode (default: gcc_phat_freq)")
    p.add_argument("--n_avg_samples", type=int, default=10,
                  help="Number of averaging samples (default: 10)")
    p.add_argument("--n_dft_bins", type=int, default=4096,
                  help="Number of DFT bins (default: 4096)")
    p.add_argument("--freq_min", type=int, default=300,
                  help="Minimum frequency in Hz (default: 300)")
    p.add_argument("--freq_max", type=int, default=3000,
                  help="Maximum frequency in Hz (default: 3000)")
    ...
    for i, (idx, row) in enumerate(rows, 1):
        ...
        try:
            res = process_row(row, idx, plots=args.plots, outdir=outdir,
                              ...
                              srp_grid_cells=args.srp_grid_cells, srp_mode=args.srp_mode,
                              n_avg_samples=args.n_avg_samples, n_dft_bins=args.n_dft_bins,

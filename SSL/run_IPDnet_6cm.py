...
class MyDataModule(LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(dataset_train, sampler=MyDistributedSampler(dataset=dataset_train, seed=2, shuffle=True),
                          batch_size=self.batch_size[0], num_workers=self.num_workers)

    def val_dataloader(self) -> DataLoader:
        return DataLoader(dataset_val, sampler=MyDistributedSampler(dataset=dataset_val, seed=2, shuffle=False),
                          batch_size=self.batch_size[1], num_workers=self.num_workers)

    def test_dataloader(self) -> DataLoader:
        return DataLoader(dataset_test, sampler=MyDistributedSampler(dataset=dataset_test, seed=2, shuffle=False),
                          batch_size=self.batch_size[1], num_workers=self.num_workers)
...
class MyModel(LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if batch_idx % 10 == 0:
            print(f"Epoch {self.current_epoch} | Batch {batch_idx}/{self.trainer.num_training_batches} "
                  f"({100*batch_idx/self.trainer.num_training_batches:.1f}%) | Loss: {loss.item():.4f}")
    ...
    def validation_step(self, batch, batch_idx: int):
        ...
        # Compute metrics BEFORE slicing (needs full 9 channels)
        ...
        # Compute loss (slices predictions internally to match ground truth)
        loss = self.cal_loss(pred_batch=pred_batch, gt_batch=gt_batch)
        self.log("valid/loss", loss, sync_dist=True)

        return {"loss": loss, **{k: v.item() for k, v in metric.items()}}

    def on_validation_epoch_end(self):
        # Print validation metrics after each epoch
        if hasattr(self.trainer.callback_metrics, '__getitem__'):
            val_loss = self.trainer.callback_metrics.get('valid/loss', None)
            val_acc = self.trainer.callback_metrics.get('valid/ACC', None)
            val_mae = self.trainer.callback_metrics.get('valid/MAE', None)

            metrics_str = f"Epoch {self.current_epoch} metrics: "
            if val_loss is not None:
                metrics_str += f"valid/loss={val_loss:.4f}  "
            if val_acc is not None:
                metrics_str += f"valid/ACC={val_acc:.4f}  "
            if val_mae is not None:
                metrics_str += f"valid/MAE={val_mae:.4f}  "
    ...
    def cal_loss(self, pred_batch=None, gt_batch=None):
        ...
        # Model now outputs correct number of IPD pairs (nmic-1 for 'M' mode)
        # No slicing needed - pred_batch and ipd_gt should have matching shapes
    ...
    def data_preprocess(self, mic_sig_batch=None, targets_batch=None, array_gemo_data=None, vad_data_batch=None,
                        eps=1e-6):
        ...
        azi_ele = torch.cat((ele_data[:, :, np.newaxis, :].to(targets_batch), targets_batch[:, :, np.newaxis, :]),
                            dim=-2)
        ...
        if self.tar_useVAD:
            ...
            vad_batch_expand_ipd = vad_batch_copy[:, :, np.newaxis, np.newaxis, :].expand(nb, nt, nf, nmic,
                                                                                          nsrc)  # .reshape(nb,nt,512,-1,nsrc)
...
class MyCLI(LightningCLI):
    def add_arguments_to_parser(self, parser: LightningArgumentParser) -> None:
        ...
        # Modified defaults for macOS/K8s compatibility
        parser.set_defaults({"trainer.accelerator": "mps"})  # Use MPS for macOS, override with --trainer.accelerator

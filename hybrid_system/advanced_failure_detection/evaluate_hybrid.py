#!/usr/bin/env python3
"""
Evaluate Hybrid CRNN-SRP System with Advanced Failure Detection

This script:
1. Loads routing decisions from advanced failure detection (temperature + Mahalanobis)
2. Runs SRP on cases routed to SRP
3. Combines CRNN (kept) + SRP (routed) for real hybrid performance
4. Compares with CRNN-only and simple threshold baseline

Based on test_hybrid_3x12cm.py but adapted for advanced routing.
"""
...
# Import routing detector
from combined_routing import CombinedFailureDetector
...
# SRP parameters (same as previous hybrid test)
SRP_CONFIG = {
    'n_dft_bins': 16384,
    'freq_min': 300,
    'freq_max': 4000,
...
def main():
    print("="*100)
    print("HYBRID CRNN-SRP WITH ADVANCED FAILURE DETECTION")
    print("="*100)
    ...
    # Load fitted detector from pipeline results
    print("\n" + "="*100)
    print("LOADING ADVANCED ROUTING DECISIONS")
    print("="*100)

    results_path = Path("results/pipeline_results.npz")
    if not results_path.exists():
        print("ERROR: Pipeline results not found. Run run_pipeline.py first.")
        return

    pipeline_results = np.load(results_path, allow_pickle=True)
    temperature = float(pipeline_results['temperature'])
    best_conf_thresh = float(pipeline_results['best_confidence_threshold'])
    best_dist_thresh = float(pipeline_results['best_distance_threshold'])

    print(f"\nOptimal parameters from pipeline:")
    print(f"  Temperature: {temperature:.4f}")
    print(f"  Confidence threshold: {best_conf_thresh:.4f}")
    print(f"  Distance threshold: {best_dist_thresh:.2f}")

    # Reconstruct detector
    print("\nReconstructing detector...")
    train_features_path = Path("features/train_6cm_features.npz")
    ...
    print(f"  Detector fitted (temperature={detector.temperature:.4f})")

    # Get routing decisions
    print("\nGetting routing decisions...")
    routing_results = detector.evaluate_combined_routing(
        features,
    ...
    for idx in tqdm(srp_indices, desc="Running SRP"):
        ...
        if srp_pred is not None:
            srp_results.append({
                ...
                'srp_pred': srp_pred,
                'srp_error': srp_error,
                'calibrated_confidence': routing_results['confidences'][idx],
                'mahalanobis_distance': routing_results['mahalanobis_distances'][idx],
            })
    ...
    srp_output = output_dir / "advanced_hybrid_srp_results.csv"
    ...
    summary_output = output_dir / "advanced_hybrid_summary.csv"
